        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="add-form">
          ...
          <div class="modal-actions">
            ...
            <button 
              class="btn-add"
              type="submit"
              [disabled]="form.invalid || !selectedType || isLoading"
              [class.disabled]="!selectedType"
            >
              <span *ngIf="!isLoading">Agregar Equipo</span>
...
export class AddEquipoModal implements OnInit {
  @Input() isOpen = false;
  @Input() clienteId: number | null = null;
  @Output() equipoAdded = new EventEmitter<void>();
  ...
  onSubmit() {
    if (!this.selectedType || !this.clienteId) return;
